...
local function ScanCharacterInventory()
    ...
    -- 1. SCHRITT: Normale Charakter-Taschen scannen (IDs 0 bis 4)
    for bag = 0, 4 do
    ...
    -- 2. SCHRITT: Charakter-Bank scannen (ID -1 ist der Haupt-Bank-Slot, 6 bis 12 sind die Bank-Taschen)
    if isBankOpen then
        -- Wir packen alle potenziellen Bank-IDs in eine feste Liste, um Edge-Cases zu vermeiden
        local bankBagIDs = { -1, 6, 7, 8, 9, 10, 11, 12 }
        
        for _, bag in ipairs(bankBagIDs) do
            if not IsWarbandBagID(bag) then
                if C_Container and C_Container.GetContainerNumSlots then
                    local slots = C_Container.GetContainerNumSlots(bag)
                    for slot = 1, slots do
                        local itemInfo = C_Container.GetContainerItemInfo(bag, slot)
                        if itemInfo and itemInfo.itemID and itemInfo.stackCount then
                            ...
                            -- Hier gab es den Fehler:isBound wurde bei Bankplätzen ignoriert oder übersprungen
                            if itemInfo.isBound then
                                soulboundData[itemIDStr] = (soulboundData[itemIDStr] or 0) + itemInfo.stackCount
                            else
...
local function SaveAllData(isInitialBoot)
    ...
    -- Taschen werden immer überschrieben
    BankSnapshotDB.realms[realmName][charName].bags = freshBags

    -- WICHTIG: Wenn die Bank offen ist, MERGEN wir das soulbound-Ergebnis komplett neu.
    -- Wenn die Bank ZU ist, dürfen wir die seelengebundenen Items aus der Bank nicht löschen!
    if isBankOpen then
        ...
        -- Bank ist zu: Wir aktualisieren NUR den Taschen-Anteil der seelengebundenen Items.
        -- Dafür behalten wir die Items, die zuvor in der Bank als soulbound markiert waren.
        local updatedSoulbound = {}
        
        -- 1. Bestehende Bank-Items im Soulbound-Block behalten (alles was nicht in den aktuellen Taschen ist)
        if BankSnapshotDB.realms[realmName][charName].soulbound then
            for itemID, qty in pairs(BankSnapshotDB.realms[realmName][charName].soulbound) do
                -- Wenn es beim letzten Mal Bank-Exklusiv war, temporär sichern
                updatedSoulbound[itemID] = qty
            end
        end
        
        -- 2. Jetzt mit den frischen Taschen-Daten überschreiben/ergänzen
        for itemID, qty in pairs(freshSoulbound) do
            updatedSoulbound[itemID] = qty
        end
        
        BankSnapshotDB.realms[realmName][charName].soulbound = updatedSoulbound
    end
